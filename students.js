...
async function createNewStudent(studentData)
{
    ...
    try {
        ...
        if (response.ok)
        {
            ...
            div_create_new_student.innerHTML = `<p student="success">Student created successfully. The new student id is ${createdStudent.id}</p>`;
            await getAndDisplayAllStudents();
        }
        else
        {
            div_create_new_student.innerHTML = '<p student="failure">ERROR: failed to create the new student</p>';
        }
    }
    catch (error)
    {
        console.error(error);
        div_create_new_student.innerHTML = `<p student="failure">ERROR: failed to connect to the API to create the new student</p>`;
    }
}
...
async function getAndDisplayAllStudents()
{
    ...
    try
    {
        ...
        {
            div_list_of_students.innerHTML = '<p student="failure">ERROR: failed to retrieve the students.</p>';
        }
    }
    catch (error)
    {
        console.error(error);
        div_list_of_students.innerHTML = '<p students="failure">ERROR: failed to connect to the API to fetch the students data.</p>';
    }
...
async function getStudent(studentId)
{
    ...
    // if this code is reached, it means that an error occurred and the student could not be retrieved
    return null;
}
...
async function updateStudent(studentData)
{
    ...
    try
    {
        ...
        if (response.ok)
        {
            div_update_student_details.innerHTML = '<p student="success">Student updated successfully</p>';
            await getAndDisplayAllStudents();
        }
        else
        {
            div_update_student_details.innerHTML = '<p student="failure">ERROR: failed to update the student</p>';
        }
    }
    catch (error)
    {
        console.error(error);
        div_update_student_details.innerHTML = `<p student="failure">ERROR: failed to update the student with id ${studentData.id}</p>`;
    }
}
...
async function deleteStudent(studentId)
{
    ...
    try
    {
        ...
        if (response.ok)
        {
            div_delete_student.innerHTML = `<p student="success">Student with id ${studentId} deleted successfully</p>`;
            await getAndDisplayAllStudents();
        }
        else
        {
            div_delete_student.innerHTML = `<p student="failure">ERROR: failed to delete the student with id ${studentId}</p>`;
        }
    }
    catch (error)
    {
        console.error(error);
        div_delete_student.innerHTML = `<p student="failure">ERROR: failed to connect to the API to delete the student with id ${studentId}</p>`;
    }
}
...
function renderStudentAsHTML(studentAsJSON)
{
    ...
    return `<div student="show-student-in-list" data-id="${studentAsJSON.id}">
        <p>Student ID (this is just for debugging): ${studentAsJSON.id}</p>
        <p>Student firstName: ${studentAsJSON.firstName}</p>
...
async function handleShowStudentDetailsEvent(event)
{
    ...
    {
        div_show_student_details.innerHTML = `<p student="failure">ERROR: failed to retrieve the student with id ${studentId}</p>`;
    }
    else
    {
...
{
    console.log({studentAsJSON});
    div_show_student_details.innerHTML = `<div student="show-student-details" data-id="${studentAsJSON.id}">
        <p>Student ID (this is just for debugging): ${studentAsJSON.id}</p>
        <p>Student First Name: ${studentAsJSON.firstName}</strong></p>
        <p>Student Last Name: ${studentAsJSON.lastName}</p>
        <p>Max Birth Date: ${studentAsJSON.birthDate}</p>
    </div>`;
}
